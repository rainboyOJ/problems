...
bool check(int a,int b) {
    ...
    int lst2 = last2[b];
    while(lst1 != -1 || lst2 != -1){
        if( info1[lst1].cnt[a] != info2[lst2].cnt[a])
            return 0;
        if( info1[lst1].cnt[b] != info2[lst2].cnt[b])
            return 0;
        lst1 = info1[lst1].pre;
        lst2 = info2[lst2].pre;
    }
...
int main (int argc, char *argv[]) {
    ...
    pre_deal(s1,l1,info1);
    for(int i = 0;i<18;i++) last1[i] = last[i];
    pre_deal(s2,l2,info2);
    for(int i = 0;i<18;i++) last2[i] = last[i];

    for(int i = 0 ;i< 18; i++)
    {
        char a = chr(i);
        for(int j = i;j < 18 ;++j ) // j: 1->18
        {
            char b = chr(j);
            string a1 = get1(s1,l1,a,b);
            string a2 = get1(s2,l2,a,b);
            ok[i][j] = a1 == a2; //字符b 与字符a是否可以配对
        }
    }
